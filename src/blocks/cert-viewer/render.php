...
<div
	...
	<div class="wp-block-scg-cert-viewer__modal" data-wp-bind--hidden="state.documentLoading">
		...
		<button class="wp-block-scg-cert-viewer__control wp-block-scg-cert-viewer__control--prev" type="button" data-wp-on-async--click="actions.prevPage" data-wp-bind--hidden="!state.hasPrevPage" wp-bind--inert="!state.hasPrevPage" aria-label="<?php esc_html_e( 'Previous page', 'scg' ); ?>">
			<?php echo wp_kses_svg( $icons['prev'] ); // @codingStandardsIgnoreLine. ?>
		</button>
		<button class="wp-block-scg-cert-viewer__control wp-block-scg-cert-viewer__control--next" type="button" data-wp-on-async--click="actions.nextPage" data-wp-bind--hidden="!state.hasNextPage" wp-bind--inert="!state.hasNextPage" aria-label="<?php esc_html_e( 'Next page', 'scg' ); ?>">
